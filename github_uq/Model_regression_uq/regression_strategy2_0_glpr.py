...
date = time.strftime('%Y%m%d_0_glpr_5001',time.localtime(time.time()))
...
for fp in fp_choice:
    ...
    pre_train_label = np.load('../0_Data/0_regre_3a4/data_split/3a4_train_labels_glpr.npy')
    pre_test_label = np.load('../0_Data/0_regre_3a4/data_split/3a4_test_labels_glpr.npy')
    ...
    # for k in range(3):
    #     maxepoch=1000*(2**k)
    lambs = [0.005,0.01,0.05,0.1,0.2,0.3,0.4,0.6,1]#0.005,0.01,0.05,0.1,0.2,0.4,0.6,1,2]
    epsilon = 1e-4
    bc_choice = [16,32,64,128,256,512]#16,,51216,,64,128,256,64,128
        ...
        for bb in bc_choice:
            ...
            for ll in lr_choice:
                ...
                if a == True:
                    ...
                    for i in range(11):
                        number=1-(i)/20
                        LnNet_models = torch.load('./%s/models/LnNet_%s_lam%s_bc%s_lr%s.pth'%(date,fp,lam,batchsize,learning_rate), map_location={'cuda:1':'cuda:1'})
